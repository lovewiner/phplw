...
{
    public function lst(){
        $data=Db::name('manager_groups')->select();
        $this->assign('data',$data);
        return $this->fetch();
    }
    public function add(){
        $gid = (int)input('get.id');
        $role = Db::name('manager_groups')->where('gid',$gid)->find();
    ...
    public function save(){
        ...
        if($gid>0){
            $res=Db::name('manager_groups')->where('gid',$gid)->update($result);
        }else{
            //检查角色是否已存在
            $_title= Db::name('manager_groups')->where('title',$result['title'])->find();
            if($_title){
                exit(json_encode(array('code'=>1,'msg'=>'角色已存在')));
            }
            $res = Db::name('manager_groups')->insert($result);
        }
        if(!$res){
    ...
    public function delete(){
        $gid = input('post.id');
        $res = Db::name('manager_groups')->where('gid',$gid)->delete();
        if(!$res){
